...
cdef inline double _pair(double dth) noexcept nogil:
    cdef double c = cos(dth)
    return 0.5 * (1.0 - 3.0 * c * c)

cdef double _one_energy_mv(double[:, :] a, int ix, int iy, int nmax) noexcept nogil:
    cdef int ixp = ix + 1
    cdef int ixm = ix - 1
    cdef int iyp = iy + 1
    cdef int iym = iy - 1
    if iyp >= nmax: iyp -= nmax
    if iym < 0: iym += nmax
    cdef double en = 0.0
    cdef double ang
    ang = a[ix, iy] - a[ixp, iy]; en += _pair(ang)
    ang = a[ix, iy] - a[ixm, iy]; en += _pair(ang)
    ang = a[ix, iy] - a[ix, iyp]; en += _pair(ang)
    ang = a[ix, iy] - a[ix, iym]; en += _pair(ang)
    return en

cdef double _energy_local_mv(double[:, :] a, int nmax, int rows) noexcept nogil:
    cdef int i, j
    cdef double s = 0.0
    for i in range(1, rows + 1):
        for j in range(nmax):
            s += _one_energy_mv(a, i, j, nmax)
    return s

cdef void _order_local_mv(double[:, :] a, int nmax, int rows, double* q00, double* q11, double* q01) noexcept nogil:
    cdef int i, j
    cdef double c, s, th
    cdef double tq00 = 0.0
    cdef double tq11 = 0.0
    cdef double tq01 = 0.0
    cdef double scale = 0.5 / (nmax * nmax)
    for i in range(1, rows + 1):
        for j in range(nmax):
            th = a[i, j]
            c = cos(th); s = sin(th)
...
cdef int _mc_half_sweep_interior_mv(double[:, :] a, double[:, :] angles, double[:, :] uniforms, int nmax, int rows, int start_row, int end_row, int parity, int g_start, double Ts) noexcept nogil:
    cdef int i_local, j, g_i, j_start, acc = 0
    cdef double ang, u, en0, en1, oldv, dE, boltz
    for i_local in range(start_row, end_row + 1):
        ...
        for j in range(j_start, nmax, 2):
            ...
            en0 = _one_energy_mv(a, i_local, j, nmax)
            oldv = a[i_local, j]
            a[i_local, j] = oldv + ang
            en1 = _one_energy_mv(a, i_local, j, nmax)
            if en1 <= en0:
                acc += 1
            else:
                dE = en1 - en0
...
def _one_energy(a, ix, iy, nmax):
    cdef double[:, :] A = a
    return _one_energy_mv(A, <int>ix, <int>iy, <int>nmax)

def _energy_local(a, nmax, rows):
    cdef double[:, :] A = a
    return _energy_local_mv(A, <int>nmax, <int>rows)

def _order_local(a, nmax, rows):
    cdef double[:, :] A = a
    cdef double q00, q11, q01
    _order_local_mv(A, <int>nmax, <int>rows, &q00, &q11, &q01)
    return q00, q11, q01

def _mc_half_sweep_interior(a, angles, uniforms, nmax, rows, start_row, end_row, parity, g_start, Ts):
    cdef double[:, :] AA = a
    cdef double[:, :] AN = angles
    cdef double[:, :] UU = uniforms
    return _mc_half_sweep_interior_mv(AA, AN, UU, <int>nmax, <int>rows, <int>start_row, <int>end_row, <int>parity, <int>g_start, <double>Ts)
...
def get_order(arr_global_like, nmax):
    ...
    Q = np.array([[vec[0], vec[2], 0.0], [vec[2], vec[1], 0.0], [0.0, 0.0, -0.5]])
...
def main(iterations, nmax, Ts, pflag):
    ...
    if rank == 0:
        print(f"MPI: Size: {nmax}, Steps: {iterations}, T*: {Ts:.3f}: Order: {order[iterations-1]:.3f}, Time: {runtime:.6f} s")
