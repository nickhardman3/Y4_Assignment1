...
def all_energy(arr, nmax):
    ...
    for i in range(nmax):
        for j in range(nmax):
            ...
            up = arr[(i - 1) % nmax, j]
            down = arr[(i + 1) % nmax, j]
            e += 0.5 * ((1 - 3 * np.cos(arr[i, j] - right)**2)
                      + (1 - 3 * np.cos(arr[i, j] - left)**2)
                      + (1 - 3 * np.cos(arr[i, j] - up)**2)
                      + (1 - 3 * np.cos(arr[i, j] - down)**2))
    return e
...
def one_energy_vector(arr, ix, nmax):
    ...
    for iy in range(nmax):
        ...
        left = row[(iy - 1) % nmax]
        up = arr[(ix - 1) % nmax, iy]
        down = arr[(ix + 1) % nmax, iy]
        en[iy] = 0.5 * ((1 - 3 * np.cos(row[iy] - right)**2)
                      + (1 - 3 * np.cos(row[iy] - left)**2)
                      + (1 - 3 * np.cos(row[iy] - up)**2)
                      + (1 - 3 * np.cos(row[iy] - down)**2))
    return en
